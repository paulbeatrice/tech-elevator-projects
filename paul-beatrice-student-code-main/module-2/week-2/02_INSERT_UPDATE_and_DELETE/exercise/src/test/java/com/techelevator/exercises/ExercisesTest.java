...
public class ExercisesTest {
	...
	private void compareData(DefaultTableModel dtExpected, SqlRowSet sqlRSVerify) {
		...
		sqlRSVerify.last();
		assertEquals(dtExpected.getRowCount(), sqlRSVerify.getRow(),
				"Couldn't find the expected data after running your SQL. Make sure you've spelled everything correctly.");
		...
		while (sqlRSVerify.next()) {
			for (String colNameExpected : colNamesExpected) {
				...
				assertEquals(valExpected, valVerify,
						"Data returned doesn't match data expected for row " + rowIndex + " in column '" + colNameExpected + "'.");
			}
		}
	}
