...
public class ExercisesTest {
	...
	private void checkRowCount(ResultSet rsExpected, ResultSet rsActual, String hint) {
		try {
			...
			if (hint.isBlank()) {
				assertEquals(rowCountExpected, rowCountActual,
						"Expected row count doesn't match actual row count.");
			} else {
				assertEquals(rowCountExpected, rowCountActual,
						"Expected row count doesn't match actual row count." + System.lineSeparator() + hint + System.lineSeparator());
			}
		...
		try {
			...
			while (rsExpected.next() && rsActual.next()) {
				for (String colNameExpected : colNamesExpected) {
					...
					assertEquals(valExpected, valActual, message);
				}
			}
		} catch (SQLException ex) {
			fail(ex.getMessage());
