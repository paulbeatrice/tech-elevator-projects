    ...
    public static void main(String[] args) {
        ...
        try (Scanner fileInput = new Scanner(bookFile)) {
            ...
            while (fileInput.hasNextLine()) {
                ...
                /*Step 4: Skip the header information before the book content
                 */

                if (lineOfText.startsWith(BEGIN_MARKER)) {
                    inBookText = true;
                    continue; //no need the process this line....go to the next
                }
                if(lineOfText.startsWith(END_MARKER)) {
                    break; // Once the program finds the end break the loop
                }
                if(inBookText) {
