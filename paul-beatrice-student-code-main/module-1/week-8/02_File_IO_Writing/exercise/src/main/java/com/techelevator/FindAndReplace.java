...
public class FindAndReplace {
    ...
    public void writeReplacedTextFile(File sourceFile, File destinationFile, String searchTerm, String replacementTerm) throws IOException {
        ...
        //String builder to accumulate the adjust content
        StringBuilder adjustedContent = new StringBuilder();


        //Process each line separately to handle case sensitivity and line breaks
        for (String line : lines) {
            //Only replace if the search term(s) is fully on one line
            if (line.contains(searchTerm)) {
                line = line.replace(searchTerm, replacementTerm);
            }
            //Append the (possibly adjusted content) to the content with a newLine
            adjustedContent.append(line).append(System.lineSeparator());
        }

        //Write the adjusted content to the destination file, overwriting it if it exists
        try (FileWriter writer = new FileWriter(destinationFile)) {
            writer.write(adjustedContent.toString());
        }

        System.out.println("The file has successfully been written with the adjusted content.");
