...
public class FizzWriter {
    ...
    public void writeFizzBuzzFile(File destinationFile, int startNumber, int endNumber, String fizzValue, String buzzValue) throws IOException {
        ...
        //Set up BufferedWriter to write the file and overwrite any existing content

        try(BufferedWriter writer = new BufferedWriter(new FileWriter(destinationFile, false))) {
            for (int i = startNumber; i <= endNumber ; i++) {
                String output;

                //Determine what to write for the current number
                if (i % 3 == 0 && i % 5 == 0) {
                    output = fizzValue + buzzValue; //Both are divisible by 3 & 5
                } else if (i % 3 == 0) {
                    output = fizzValue; // For numbers divisible by 3
                } else if (i % 5 == 0) {
                    output = buzzValue; // For numbers divisible by 5
                } else {
                    output = Integer.toString(i); // Not divisible by 3 0r 5
                }

                // Write the output to the file and move on to the next line
                writer.write(output);
                writer.newLine();
            }
        }
