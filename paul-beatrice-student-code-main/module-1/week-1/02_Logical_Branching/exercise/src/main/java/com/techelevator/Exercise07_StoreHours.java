...
public class Exercise07_StoreHours {
    ...
    public boolean isStoreOpen(int currentHour, char currentDay, boolean isSummer) {
        //Returns correct open status when Monday or Friday
        if ((currentHour >=8 && currentHour <17) && (currentDay =='M' || currentDay == 'F')&& isSummer){
            return true;
        }
        if ((currentHour >=8 && currentHour <17) && (currentDay =='M' || currentDay == 'F') && !isSummer){
            return true;
        }
        if ((currentHour <8 && currentHour >0) && (currentDay =='M' || currentDay == 'F') && !isSummer){
            return false;
        }
        if ((currentHour >=8 && currentHour <20) && currentDay=='W' && isSummer){
            return true;
        }
        if ((currentHour >=8 && currentHour <17) && currentDay=='W' && !isSummer){
            return true;
        }
        if ((currentHour >=17 && currentHour <20) && currentDay=='W' && !isSummer){
            return false;
        }
        if ((currentHour >=9 && currentHour <15) && currentDay=='S' & isSummer){
            return true;
        }
        if ((currentHour >=8 && currentHour <20) && (currentDay == 'S') && !isSummer){
            return false;
        }
        return false;
    }
