...
public class Exercise03_ShippingTotal {
    ...
    public double calculateShippingTotal(int weightPounds) {
        // weight GT 40
        if (weightPounds >40) {
            return 0.75 * weightPounds;
        }
        // weight LTE 40
        if (weightPounds <=40){
            return 0.50 * weightPounds;
        }
        return 0.50;
    }
    ...
    public double calculateShippingTotal(int weightPounds, boolean hasDiscount) {
        ...
        if (weightPounds >40 && hasDiscount) {
            shippingTotal= (0.75 * weightPounds);
            discountedPrice= (shippingTotal) * 0.10;
            return shippingTotal - discountedPrice;
        }
        if (weightPounds >40 && !hasDiscount) {
            return (0.75 * weightPounds);
        }
        // weight LTE 40
        if (weightPounds <=40 && hasDiscount) {
            shippingTotal= (0.50 * weightPounds);
            discountedPrice= (shippingTotal) * 0.10;
            return shippingTotal - discountedPrice;
        }
        if (weightPounds <=40 && !hasDiscount) {
