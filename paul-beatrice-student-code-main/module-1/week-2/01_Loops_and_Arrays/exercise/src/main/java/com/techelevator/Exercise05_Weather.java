...
public class Exercise05_Weather {
    ...
    */
    public int hottestDay(int[] dailyHighs) {
        ...
        int tempMax = dailyHighs[0];

        for (int i = 0; i < dailyHighs.length; i++) {
            if (tempMax < dailyHighs[i]) {
                tempMax = dailyHighs[i];
            }
        }
        return tempMax;
    }
    ...
    public int[] fixTemperatures(int[] temperatures){
        ...
        int[] fixTemperatures = new int [temperatures.length];

        for (int i = 0; i < temperatures.length; i++) {
            if (i % 2 == 0) {
                fixTemperatures[i] = temperatures[i] + 2;
            } else {
                fixTemperatures[i] = temperatures[i];

            }
        }
        return fixTemperatures;
