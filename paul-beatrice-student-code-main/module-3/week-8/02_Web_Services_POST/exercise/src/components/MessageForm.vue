   ...
   async submitForm() {
      ...
      try {
        ...
        if (this.editMessage.id === 0) {
          response = await messageService.create(this.editMessage);
          if (response.status === 201) {
            this.$router.push({
              name: "TopicDetailsView",
              params: { id: this.editMessage.topicId }
            });
          } else {
            this.handleErrorResponse(response, "adding");
          }
        } else {
          response = await messageService.update(this.editMessage.id, this.editMessage);
          if (response.status === 200) {
            this.$router.push({
              name: "MessageDetailsView",
              params: { id: this.editMessage.id }
            });
          } else {
        ...
        const verb = this.editMessage.id === 0 ? "adding" : "updating";
        this.handleErrorResponse(error, verb);
      }
    },
    cancelForm() {
      this.$router.back();
